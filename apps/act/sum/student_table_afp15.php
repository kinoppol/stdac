...
if(get_system_config("active_assembly")=='active')array_push($head,'กิจกรรมของชมรม');
array_push($head,'กิจกรรมวิทยาลัย');
array_push($head,'บันทึกพิเศษ');
array_push($head,'ผลการประเมิน');
...
foreach($ac_data as $row){
    ...
    $table_data[$i]=array(
        ...
        'stu_fullname'=>$row['stu_fname'].' '.$row['stu_lname'],
    ...
    if(get_system_config("active_morning_ceremony")=='active'){
        //$table_data[$i]['sumMorningCeremony']='<span '.$morning_ceremony_color.'>'.$total_sign_morning_ceremony.'/'.$date_morning_ceremony.' ครั้ง ('.number_format($morning_ceremony_percentage,2).'%)</span>';
        $table_data[$i]['sumMorningCeremony']=$morning_ceremony_grade=='fail'?'ไม่ผ่าน':'ผ่าน';//'<span '.$morning_ceremony_color.'>'.$total_sign_morning_ceremony.'/'.$date_morning_ceremony.' ครั้ง ('.number_format($morning_ceremony_percentage,2).'%)</span>';
    }
    if(get_system_config("active_assembly")=='active'){
        //$table_data[$i]['sumAssembly']='<span '.$assembly_color.'>'.$total_sign_assembly.'/'.$date_assembly.' ครั้ง ('.number_format($assembly_percentage,2).'%)</span>';
        $table_data[$i]['sumAssembly']=$assembly_grade=='fail'?'ไม่ผ่าน':'ผ่าน';
    }
    //$table_data[$i]['sumact']='<span '.$activity_color.'>'.$total_signact.'/'.count($act_id).' ครั้ง ('.number_format($percentage,2).'%)</span>';
    $table_data[$i]['sumact']=$act_grade=='fail'?'ไม่ผ่าน':'ผ่าน';
    $table_data[$i]['special_note']='';

    $table_data[$i]['grade']=($act_grade=='fail'||$morning_ceremony_grade=='fail'||$morning_ceremony_grade=='fail')?'ไม่ผ่าน':'ผ่าน';
